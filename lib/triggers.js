...
function logTriggerAfterHook(triggerType, className, input, auth) {
  var cleanInput = _logger.logger.cleanAndTruncateLogMessage(JSON.stringify(input));
...
function logTriggerSuccessBeforeHook(triggerType, className, input, result, auth) {
  var cleanInput = _logger.logger.cleanAndTruncateLogMessage(JSON.stringify(input));
  var cleanResult = _logger.logger.cleanAndTruncateLogMessage(JSON.stringify(result));
  _logger.logger.info(triggerType + ' triggered for ' + className + ' for user ' + userIdForLog(auth) + ':\n  Input: ' + cleanInput + '\n  Result: ' + cleanResult, {
    className: className,
    triggerType: triggerType,
    user: userIdForLog(auth)
